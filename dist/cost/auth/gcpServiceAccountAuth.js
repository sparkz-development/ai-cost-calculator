 ...
 */
async function signJWT(content, privateKey) {
 ...
 */
async function getGoogleAccessToken(serviceAccountJson, orgId, scopes = ["https://www.googleapis.com/auth/cloud-platform"], cacheProvider) {
    let serviceAccount;
    try {
        serviceAccount = JSON.parse(serviceAccountJson);
    }
    catch (error) {
        throw new Error(`Invalid service account JSON: ${error instanceof Error ? error.message : "Parse error"}`);
    }
 ...
 */
async function generateGoogleAccessToken(serviceAccount, scopes) {
    // Create JWT header
